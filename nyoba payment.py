...
class Payment(tk.Frame):
    ...
    def create_layout(self, payment_option):
        ...
        trip_detail = 'Trip Detail :' + route['source_code'] + ' - ' + route['destination_code'] + ' ' + date
        ...
        selected_label = tk.Label(payment_option_frame, text='Selected seat: ' + ','.join(seat))
    ...
    def book():
        write_update('data/order.csv', order_df, self.order_id - 1,'is_paid', True)
        popup_showinfo('Payment', 'Payment','Payment Success\n thank youfor booking travel with us!')
        self.destroy()
    ...
    def reload(self, payment_option):
          self.destroy()
          return Payment(self.master, self.order_id, self.booking_id, payment_option)
